from django.db import models

# Create your models here.
class UserInfoTest(models.Model):
    '''
    create table appTest_userinfotest(
        id bigint  auto_increment primary key,
        name varchar(32),
        password varchar(64),
        age int
    );
    '''
    name = models.CharField(max_length=32)
    password = models.CharField(max_length=64)
    age = models.IntegerField()

    # 新增時, 為了不讓新的column無數值可填入,
    # 1 -> 在終端指定隨意值,
    # 2-1 -> 給定default值 --> age = models.IntegerField(default=18)
    # 2-2 -> 允許為空 --> age = models.IntegerField(null=True, blank=True)

class StockTest(models.Model):

    date = models.DateTimeField()
    stock = models.CharField(max_length=20)
    open_p = models.FloatField()
    high_p = models.FloatField()
    low_p = models.FloatField()
    close_p = models.FloatField()
    adj_close = models.FloatField()
    volume = models.IntegerField()

# insert into appTest(date, stock, open_p, high_p, low_p, close_p, volume) values(同下....)

# StockTest.objects.create(date='2021-10-25', stock='2330.TW',
# open_p=597.0, high_p=597.0, low_p=590.0, close_p=593.0, adj_close=587.372986, volume=16785568)

# ======================================================================
# DRF0409
from django.utils import timezone


class Article(models.Model):
    title = models.CharField(max_length=100)
    body = models.TextField()
    created = models.DateTimeField(default=timezone.now)
    updated = models.DateTimeField(auto_now=True)

    def __str__(self):
        return self.title


# class Articletest(models.Model):
#     title = models.CharField(max_length=100)
#     body = models.TextField()
#     created = models.DateTimeField(default=timezone.now)
#     updated = models.DateTimeField(auto_now=True)

#     def __str__(self):
#         return self.title

# ======================================================================
# 0410 DRF前置補充
# from django.contrib.auth.models import User

# class ArticlePost(models.Model):
#     # 文章作者。参数 on_delete 用于指定数据删除的方式
#     author = models.ForeignKey(User, on_delete=models.CASCADE)
#     # 文章标题。models.CharField 为字符串字段，用于保存较短的字符串，比如标题
#     title = models.CharField(max_length=100)
#     # 文章正文。保存大量文本使用 TextField
#     body = models.TextField()
#     # 文章创建时间。参数 default=timezone.now 指定其在创建数据时将默认写入当前的时间
#     created = models.DateTimeField(default=timezone.now)
#     # 文章更新时间。参数 auto_now=True 指定每次数据更新时自动写入当前时间
#     updated = models.DateTimeField(auto_now=True)

#     # 内部类 class Meta 用于给 model 定义元数据
#     class Meta:
#         # ordering 指定模型返回的数据的排列顺序
#         # '-created' 表明数据应该以倒序排列
#         ordering = ('-created',)

#     # 函数 __str__ 定义当调用对象的 str() 方法时的返回值内容
#     def __str__(self):
#         # return self.title 将文章标题返回
#         return self.title
# ======================================================================